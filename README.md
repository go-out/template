...
どちらの場合も、`info.markdown`やcoverの相対パスは、**リポジトリルート基準**で書きます（例: `date/xxx.md`、`osaka/city24/xxx.md`）。同じJSONが`date/index.html`から直接見られる場合と、region側の`eventsJSON`経由でルートのmapページから読み込まれる場合の両方があり、後者は相対パスに一切プレフィックスを付けずそのまま`fetch`するため、ルート基準で統一しておかないと参照がずれます。
